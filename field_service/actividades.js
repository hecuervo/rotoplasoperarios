...
function getActividadById(req, res) {
  ...
  db.one('SELECT plantas.sfid as id_planta, plantas.name as nombre_planta, accounts.sfid as id_cliente, accounts.name as nombre_cliente, usuarios.name as tecnico_asignado, workorders.workordernumber, actividades.name as nombre_actividad, actividades.cantidad__c, actividades.clavecontrol__c, actividades.duracion__c, actividades.Estatus__c, actividades.OrdenServicio__c, actividades.PuestoTrabajo__c, actividades.TrabajoReal__c, workorders.worktypeid, worktypes.name as tipo_trabajo, workorders.RecordTypeId, recordtypes.name as tipo_registro FROM salesforcerotoplas.actividados__c as actividades INNER JOIN salesforcerotoplas.workorder as workorders on workorders.sfid = actividades.ordenservicio__c INNER JOIN salesforcerotoplas.account as accounts on accounts.sfid = workorders.accountid INNER JOIN salesforcerotoplas.planta__c plantas on accounts.planta_del_del__c = plantas.sfid LEFT JOIN salesforcerotoplas.usuarioapp__c as usuarios on usuarios.sfid = workorders.usuarioapp__c INNER JOIN salesforcerotoplas.worktype as worktypes on worktypes.sfid = workorders.worktypeid INNER JOIN salesforcerotoplas.recordtype as recordtypes on recordtypes.sfid = actividades.recordtypeid WHERE actividades.sfid = $1', id)
    .then(function(data) {
      res.status(200).send({
          data: data
        });
