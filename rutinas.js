...
function getRutinasUsuario(req, res) {
  ...
  db.many('SELECT rutina.name, preguntarutina.turno__c, tiporutina.nombre__c, actividadrutina.id_rutinas_heroku__c, rutina.rutaimagen__c, rutina.observacion__c, rutina.idtiporutina__c, rutina.usuarioapp__c, rutina.idplanta__c, planta.formato__c, planta.determinante__c, createddate_heroku__c FROM ' + process.env.DATABASE_SCHEMA + '.actividadrutina__c as actividadrutina RIGHT JOIN ' + process.env.DATABASE_SCHEMA + '.preguntarutina__c as preguntarutina ON (preguntarutina.sfid = actividadrutina.id_pregunta_rutina__c) INNER JOIN ' + process.env.DATABASE_SCHEMA + '.rutinas__c as rutina on (rutina.id_rutinas_heroku__c = actividadrutina.id_rutinas_heroku__c) INNER JOIN ' + process.env.DATABASE_SCHEMA + '.planta__c as planta on (planta.sfid = rutina.idplanta__c) INNER JOIN ' + process.env.DATABASE_SCHEMA + '.tiporutina__c as tiporutina on (tiporutina.sfid = rutina.idtiporutina__c) WHERE idplanta__c = $1 and usuarioapp__c = $2 GROUP BY rutina.name, preguntarutina.turno__c, tiporutina.nombre__c, actividadrutina.id_rutinas_heroku__c, rutina.rutaimagen__c, rutina.observacion__c, rutina.idtiporutina__c, rutina.usuarioapp__c, rutina.idplanta__c, rutina.createddate, planta.formato__c, planta.determinante__c, createddate_heroku__c ORDER BY rutina.createddate_heroku__c DESC', [idPlanta, idOperador])
    .then(function (data) {
      res.status(200).send({
          data: data
        });
...
function createActividadRutina(id_rutinas_heroku__c, actividadesRutina, callback) {
  for(var i in actividadesRutina) {
    let foto1 = actividadesRutina[i].foto1__c != '' ? process.env.AZURE_URL + 'rutina' + id_rutinas_heroku__c + '/' + actividadesRutina[i].foto1__c : '';
    let foto2 = actividadesRutina[i].foto2__c != '' ? process.env.AZURE_URL + 'rutina' + id_rutinas_heroku__c + '/' + actividadesRutina[i].foto2__c : '';
