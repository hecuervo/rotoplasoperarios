...
function getRutinasUsuario(req, res) {
  ...
  db.many('SELECT rutina.id_rutinas_heroku__c, rutina.name, preguntarutina.turno__c, tiporutina.nombre__c, actividadrutina.idrutina__c, rutina.rutaimagen__c, rutina.observacion__c, rutina.idtiporutina__c, rutina.usuarioapp__c, rutina.idplanta__c, planta.formato__c, planta.determinante__c FROM ' + dbConfig.schema + '.actividadrutina__c as actividadrutina RIGHT JOIN ' + dbConfig.schema + '.preguntarutina__c as preguntarutina ON (preguntarutina.sfid = actividadrutina.id_pregunta_rutina__c) INNER JOIN ' + dbConfig.schema + '.rutinas__c as rutina on (rutina.sfid = actividadrutina.idrutina__c) INNER JOIN ' + dbConfig.schema + '.planta__c as planta on (planta.sfid = rutina.idplanta__c) INNER JOIN ' + dbConfig.schema + '.tiporutina__c as tiporutina on (tiporutina.sfid = rutina.idtiporutina__c) WHERE idplanta__c = $1 and usuarioapp__c = $2 GROUP BY rutina.id_rutinas_heroku__c, rutina.name, preguntarutina.turno__c, tiporutina.nombre__c, actividadrutina.idrutina__c, rutina.rutaimagen__c, rutina.observacion__c, rutina.idtiporutina__c, rutina.usuarioapp__c, rutina.idplanta__c, rutina.createddate, planta.formato__c, planta.determinante__c ORDER BY rutina.createddate_heroku__c DESC', [idPlanta, idOperador])
    .then(function (data) {
      res.status(200).send({
          data: data
        });
