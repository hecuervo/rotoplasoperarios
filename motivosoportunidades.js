...
function getAllDescripcionesFalla(req, res,next) {
  db.many('select * from  ' + process.env.DATABASE_SCHEMA + '.descripciondefalla__c ')
    .then(function (data) {
      console.log(data);
        res.status(200).send({
          data: data
    ...
    .catch(function(err) {
      if(err.received == 0){
        res.status(404).send({message:'No se ha encontrado las descripciones de las fallas.'});
      }else{
        res.status(500).send({message:'Error en el servidor'});
      }
    });
...
function getAllDesestabilizaciones(req, res) {
  db.many('select * from  ' + process.env.DATABASE_SCHEMA + '.motivodedesestabilizacion__c')
    .then(function (data){
      res.status(200).send({
          data: data
      });
...
getAllDescripcionesFalla: getAllDescripcionesFalla,
getAllDesestabilizaciones: getAllDesestabilizaciones
